<template>
    <!-- Barre de navigation principale -->
    <nav class="navbar" role="navigation" aria-label="main navigation">
        <div class="navbar-brand">
            <!-- Logo avec lien vers la page d'accueil -->
            <router-link to="/" class="navbar-item">
                <img :src="locafilmLogo" width="112" height="28" alt="Locafilm Logo">
            </router-link>

            <!-- Bouton hamburger pour les appareils mobiles -->
            <a role="button" class="navbar-burger burger" aria-label="menu" aria-expanded="false"
                data-target="navbarBasicExample" @click="showMobileMenu = !showMobileMenu">
                <span aria-hidden="true"></span>
                <span aria-hidden="true"></span>
                <span aria-hidden="true"></span>
            </a>
        </div>

        <!-- Menu de navigation -->
        <div id="navbarBasicExample" class="navbar-menu" :class="{ 'is-active': showMobileMenu }">
            <div class="navbar-start">
                <!-- Liens vers les pages d'administration -->
                <router-link :to="{ name: 'ListeUtilisateurs', }" class="navbar-item">Utilisateurs</router-link>
                <router-link :to="{ name: 'ListeFilms', }" class="navbar-item">Films</router-link>
            </div>
            <div class="navbar-start">
                <!-- Lien vers la page de chat -->
                <router-link :to="{name: 'Chat'}" class="navbar-item">Chat</router-link>
            </div>

            <!-- Boutons de déconnexion -->
            <div class="navbar-end">
                <div class="navbar-item">
                    <div class="buttons">
                        <!-- Bouton pour se déconnecter -->
                        <button class="button is-danger is-outlined" @click="deconnexion">Déconnexion</button>
                    </div>
                </div>
            </div>
        </div>
    </nav>
</template>

<script>
import axios from 'axios';
import locafilmLogo from '/images/logos/locafilm.png';
import Cookies from 'js-cookie';

export default {
    name: 'MenuAdmin.vue',
    data() {
        return {
            showMobileMenu: false, // Indicateur pour afficher/masquer le menu mobile
            locafilmLogo, // Logo de Locafilm
            isLoggedOut: false, // Indicateur de déconnexion
        };
    },
    methods: {
        async deconnexion() {
            try {
                // Récupération du cookie d'authentification
                const monCookie = Cookies.get('token');
                // Configuration de la requête Axios
                const config = {
                    headers: {},
                    withCredentials: true
                };
                if (monCookie) {
                    // Ajout du token dans les en-têtes de la requête
                    config.headers['Authorization'] = `Bearer ${monCookie}`;
                }
                // Appel à l'API pour se déconnecter
                await axios.get('http://localhost:8080/auth/deconnexion', config);
                // Suppression des cookies 'cook' et 'token'
                document.cookie = 'cook=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
                document.cookie = 'token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
                // Alerte de déconnexion réussie
                alert('Vous êtes maintenant déconnecté.');
                // Redirection vers la page de connexion
                window.location.href = '/connexion';
            } catch (erreur) {
                // Gestion des erreurs
                alert(erreur.response.data.message);
            }
        },
    },
};
</script>

<style scoped>
/* Styles spécifiques à la barre de navigation */
.navbar {
    background-color: #363636; /* Couleur de fond */
    color: white; /* Couleur du texte */
}

/* Styles des liens de navigation */
.navbar-item {
    color: white; /* Couleur du texte */
}

/* Styles des liens de navigation au survol */
.navbar-item:hover {
    color: #ccc; /* Couleur du texte au survol */
}

/* Styles du bouton hamburger */
.navbar-burger {
    color: white; /* Couleur du bouton */
}

/* Styles du bouton hamburger au survol */
.navbar-burger:hover {
    color: #ccc; /* Couleur du bouton au survol */
}
</style>
